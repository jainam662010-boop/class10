  ...
  renderPanel() {
    ...
    body.innerHTML = `
      <div style="display:flex;gap:7px;margin-bottom:12px;flex-wrap:wrap">
        <span class="tag tag-saffron">📋 ${all.length}</span>
        <span class="tag tag-terra">⏳ ${all.filter(t=>!t.done).length} pending</span>
        <span class="tag tag-sage">✅ ${all.filter(t=>t.done).length} done</span>
      </div>
      <div class="todo-add-row">
        <input class="todo-inp" id="panelIn" placeholder="Quick task…" onkeydown="if(event.key==='Enter')TodoPanel.quickAdd()">
        <button class="btn btn-saffron btn-sm" onclick="TodoPanel.quickAdd()">+</button>
      </div>
      ...
      <br>
      <a href="todo.html" class="btn btn-ghost" style="width:100%;justify-content:center">Open Task Manager →</a>`;
  },
  itemHTML(t) {
    ...
    return `<div class="todo-item ${t.done ? 'is-done' : ''}" data-id="${t.id}">
      ...
      <div style="flex:1;min-width:0">
        ...
        <div style="display:flex;gap:5px;margin-top:3px;flex-wrap:wrap;align-items:center">
          ...
          ${t.priority === 'high' ? `<span class="tag tag-terra" style="font-size:.63rem;padding:1px 6px">● High</span>` : ''}
          ${t.marks ? `<span class="tag tag-saffron" style="font-size:.63rem;padding:1px 6px">${t.marks}M</span>` : ''}
        </div>
      </div>
